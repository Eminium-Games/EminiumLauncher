...
ipcMain.handle('servers:get', async () => {
  try {
    // Ici, vous pourriez charger les serveurs depuis un fichier de configuration ou une API
    // Pour l'instant, nous retournons une liste vide pour que le client utilise les valeurs par défaut
    return { 
      ok: true, 
      servers: [] 
    };
...
ipcMain.handle('modpack:check', async (_evt, { name, version }) => {
  try {
    const modpackPath = path.join(app.getPath('userData'), 'modpacks', name, version);
    const exists = fs.existsSync(modpackPath);
    return { ok: true, installed: exists };
  } catch (error) {
    console.error('Erreur lors de la vérification du modpack:', error);
    return { ok: false, error: error.message };
  }
});

ipcMain.handle('modpack:download', async (_evt, { url, name, version }) => {
  try {
    const modpackDir = path.join(app.getPath('userData'), 'modpacks', name, version);
    const zipPath = path.join(modpackDir, 'modpack.zip');
    ...
    console.error('Erreur lors du téléchargement du modpack:', error);
    return { ok: false, error: error.message };
  }
});

// Lancement du jeu
ipcMain.handle('game:launch', async (_evt, options) => {
  try {
    const { version, serverAddress, serverPort, javaPath, javaArgs, modpack } = options;
    
    // Préparer les arguments de ligne de commande
    let args = [
      `-Xmx${javaArgs.match(/-Xmx(\d+[MG])/)[1]}`,
      `-Xms${javaArgs.match(/-Xms(\d+[MG])/)[1]}`,
      '-Djava.library.path=${natives_directory}',
      '-cp', '${classpath}',
      'net.minecraft.client.main.Main',
      '--version', version,
      '--accessToken', '0', // Token d'accès (à remplacer par un vrai token si nécessaire)
      ...
      '--uuid', '${auth_uuid}',
      '--userType', 'mojang',
      '--versionType', 'EminiumLauncher'
    ];
    
    // Ajouter l'adresse du serveur si spécifiée
    if (serverAddress) {
      args.push('--server', serverAddress);
      if (serverPort) {
        args.push('--port', serverPort);
      }
    }
    
    // Ici, vous pourriez ajouter la logique pour lancer le jeu avec les arguments
